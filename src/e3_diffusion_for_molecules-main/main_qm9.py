...
#TODO get_optim is an optimizer
# optim = get_optim(args, params)

# print(f"The model architecture is: {params}")
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        if epoch % args.test_epochs == 0:
            ...
            nll_val = test(rng, args=args, loader=dataloaders['valid'], epoch=epoch, eval_model=model_ema_dp,
                           ...
                           property_norms=property_norms, model_state=model_state)
            nll_test = test(rng, args=args, loader=dataloaders['test'], epoch=epoch, eval_model=model_ema_dp,
                            partition='Test', device=device, dtype=dtype,
                            nodes_dist=nodes_dist, property_norms=property_norms, model_state=model_state)
