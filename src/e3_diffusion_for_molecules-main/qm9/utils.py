...
def prepare_context_jax(conditioning, minibatch, property_norms):
    ...
    node_mask = jnp.expand_dims(minibatch['atom_mask'], axis=2)
    context_node_nf = 0
    context_list = []
    for key in conditioning:
        properties = minibatch[key]
        ...
        if properties.ndim == 1:
            ...
            reshaped = jnp.reshape(properties, (batch_size, 1, 1))
            reshaped = jnp.tile(reshaped, (1, n_nodes, 1))
            context_list.append(reshaped)
            context_node_nf += 1
        elif properties.ndim == 2 or properties.ndim == 3:
            # Node feature.
            ...
            if properties.ndim == 2:
                context_key = jnp.expand_dims(context_key, axis=2)

            context_list.append(context_key)
            context_node_nf += context_key.shape[2]
        else:
            raise ValueError('Invalid array size, more than 3 axes.')
    # Concatenate
