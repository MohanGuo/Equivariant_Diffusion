...
def get_item_if_possible(arr):
    return arr.item() if isinstance(arr, jax.Array) else arr
...
def save_xyz_file(path, one_hot, charges, positions, dataset_info, id_from=0, name='molecule', node_mask=None):
    ...
    # Compute atoms per molecule
    if node_mask is not None:
        atomsxmol = jnp.sum(node_mask, axis=1)
    else:
        atomsxmol = [one_hot.shape[1]] * one_hot.shape[0]

    # Loop through each batch
    for batch_i in range(one_hot.shape[0]):
        file_path = os.path.join(path, f"{name}_{batch_i + id_from:03d}.txt")
        with open(file_path, "w") as f:
            atomsxmol_batch_i = get_item_if_possible(atomsxmol[batch_i])

            f.write(f"{int(atomsxmol_batch_i)}\n\n")
            atoms = jnp.argmax(one_hot[batch_i], axis=1)
            n_atoms = int(atomsxmol_batch_i)
            for atom_i in range(n_atoms):
                atom = atoms[atom_i]
                atom = dataset_info['atom_decoder'][atom]
                bat_pos_i_tup = positions[batch_i, atom_i, 0].item(), positions[batch_i, atom_i, 1].item(), positions[batch_i, atom_i, 2].item(),
                f.write(
                    f"{atom} {bat_pos_i_tup[0]:.9f} {bat_pos_i_tup[1]:.9f} {bat_pos_i_tup[2]:.9f}\n")
...
def load_molecule_xyz(file, dataset_info):
    with open(file, encoding='utf8') as f:
        ...
        for i in range(n_atoms):
            atom = atoms[i].split()
            atom_type = atom[0]
            one_hot = one_hot.at[i, dataset_info['atom_encoder'][atom_type]].set(1)
            position = jnp.array([float(e) for e in atom[1:]], dtype=jnp.float32)
            positions = positions.at[i, :].set(position)
...
def plot_data3d(positions, atom_type, dataset_info, camera_elev=0, camera_azim=0, save_path=None, spheres_3d=False,
    ...
    if 'qm9' in dataset_info['name']:
        max_value = jnp.abs(positions).max().item()
        axis_lim = min(40, max(max_value / 1.5 + 0.3, 3.2))
        ax.set_xlim(-axis_lim, axis_lim)
        ax.set_ylim(-axis_lim, axis_lim)
        ax.set_zlim(-axis_lim, axis_lim)
...
def visualize_chain(path, dataset_info, wandb=None, spheres_3d=False, mode="chain"):
    ...
    for i in range(len(files)):
        ...
        atom_type = jnp.argmax(one_hot, axis=1)  # Using jax.numpy.argmax instead of torch.argmax
        fn = file[:-4] + '.png'
